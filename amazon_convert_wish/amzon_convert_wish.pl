...
# 打开us.xlsx文件
my $Book11 = $Excel->Workbooks->Open("E:\\tool\\amazon_convert_wish\\$filename") //  die "Can not open $filename book\n" ;	
...
my $brand   = $ptitles[0];
my $bullet_point1 = $Sheet11->range("AZ5")->{Value};
my $bullet_point2 = $Sheet11->range("BA5")->{Value};
my $bullet_point3 = $Sheet11->range("BB5")->{Value};
my $bullet_point4 = $Sheet11->range("BC5")->{Value};
my $bullet_point5 = $Sheet11->range("BD5")->{Value};
...
# 打开新文档 写数据
my $Book22 = $Excel->Workbooks->Open("E:\\tool\\amazon_convert_wish\\$newxlsx") //  die "Can not open $newxlsx book\n" ;	
